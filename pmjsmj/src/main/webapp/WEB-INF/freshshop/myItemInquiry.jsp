	...
	<div class="my-account-box-main">
		<div class="container">
			<div style="width: 100%; height: 65px;">
				<p style="font-size: 37px; font-weight: bold; margin-left: 275px;">상품
					문의</p>
			</div>
			<c:import url="my-account-left-nav.jsp" />
			<div class="QandA_Contain">
				<p>
					&nbsp;<font class="reviewTitle">Q&amp;A</font>&nbsp; <font>내가
						작성한 상품 문의</font>
				</p>
				<div class="line"></div>
				<table class="reviewTable">
					...
					<c:if test="${hash != null }">
						<c:forEach items="${hash.list}" var="iiqDTO">
							<tr>
								<td>${iiqDTO.inq_no }</td>
								<td>${iiqDTO.question_content}</td>
								<td>${iiqDTO.inq_title}</td>
								<td>${iiqDTO.mem_id }</td>
								<td>${iiqDTO.inq_reg_date }</td>
							</tr>
						</c:forEach>
					</c:if>
					<c:if test="${hash == null }">
				...
				<c:if test="${hash != null }">
					<div class="paging">
						<c:if test="${hash.pdto.presentPage!=1}">
							<a href="/myItemInquiry?presentPage=1" class="btn_arr prev">&lt;</a>
						</c:if>
						<!-- D : 활성화페이지일 경우 : on 처리 -->
						<c:forEach var="i" begin="${hash.pdto.startPage}"
							end="${hash.pdto.endPage}">
							<c:if test="${i == hash.pdto.presentPage }">
								<a href="/myItemInquiry?presentPage=${i}" id="page${i }"
									class="on">${i }</a>
							</c:if>
							<c:if test="${i != hash.pdto.presentPage }">
								<a href="/myItemInquiry?presentPage=${i}" id="page${i }">${i }</a>
							</c:if>
						</c:forEach>
						<c:if test="${hash.pdto.presentPage!=hash.pdto.endPage}">
							<a href="/myItemInquiry?presentPage=${hash.pdto.allPage}"

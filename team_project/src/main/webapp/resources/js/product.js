...
$(function(){
	...
	$(".order-btn").click(function(){
		let order_pay_option = $('input[id="order_pay_option"]:checked').val();
		let pay_bank_info = $('select[id="pay_bank_info"]').val();
		let pay_credit_name =$('select[id="pay_credit_name"]').val();
...
//	 alert("discount : "+discount);
	 let val = Number($(".numBox" + i).val());
	   $(".numBox" + i).val(val + 1);
	   let plusVal = Number($(".numBox" + i).val());
	   
	   let sumPriceCal = Number((first-(first - discount)) * plusVal);
	   let money = Number(first-(first - discount));
	   $(".sumPrice" + i).text(sumPriceCal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
	   
	   
	let cart_cnt = $(".numBox"+i).val();
	$.ajax({
		type : "POST",
		url : "/itemCount",
